...
class MilvusVectorClient(RetrievalClientBase):
    ...
    def __init__(self, endpoint_name: str | None = None):
        ...
        if not self.uri:
            error_msg = "Milvus URI is empty. Please check if you have set MILVUS_ENDPOINT env var or milvus.api_endpoint_env in config_retrieval.yaml properly."
        ...
        with self._client_lock:
            if client_key not in self._milvus_clients:
                ...
                try:
                    ...
                    logger.error(f"Failed to connect to Milvus at {self.uri}: {e!s}")
                    raise

        return self._milvus_clients[client_key]

    def collection_exists(self, collection_name: str | None = None,
    ...
    def ensure_collection_exists(self, collection_name: str | None = None,
                               embedding_size: str = "small") -> bool:
        """
    ...
    async def delete_documents_by_site(self, site: str,
        ...
        try:
            ...
            logger.error(f"Error deleting documents for site {site}: {e!s}")
            return 0
    ...
    def _delete_documents_by_site_sync(self, site: str, collection_name: str,
        ...
        try:
            ...
            logger.error(f"Error in _delete_documents_by_site_sync for site {site}: {e!s}")
            raise
    ...
    async def search(self, query: str, site: Union[str, list[str]],
                   num_results: int = 50, collection_name: str | None = None,
                   query_params: dict[str, Any] | None = None, **kwargs) -> list[list[str]]:
        """
        ...
        try:
            ...
            logger.exception("Error in Milvus search")
    ...
    def _search_sync(self, query: str, site: Union[str, list[str]], num_results: int,
                   ...
                   query_params: dict[str, Any] | None) -> list[list[str]]:
        ...
        try:
            ...
            if res and len(res) > 0:
                for item in res[0]:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to parse text field as JSON: {e!s}")
                        continue
            ...
            logger.exception("Error in _search_sync")
            raise

    async def search_by_url(self, url: str, collection_name: str | None = None) -> list[str] | None:
        """
    ...
    def _search_by_url_sync(self, url: str, collection_name: str) -> list[str] | None:
    ...
    async def get_sites(self, collection_name: str | None = None,
        ...
        try:
            ...
            logger.exception(f"Error retrieving sites from collection '{collection_name}': {e!s}")
            logger.log_with_context(
                LogLevel.ERROR,
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.error(f"Error querying batch at offset {offset}: {e!s}")
                    break
            ...
            logger.error(f"Error in _get_sites_sync for collection '{collection_name}': {e!s}")
            raise
