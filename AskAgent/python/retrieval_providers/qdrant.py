...
class QdrantVectorClient(RetrievalClientBase):
    ...
    async def create_collection(self, collection_name: str | None = None,
        ...
        try:
            ...
            logger.error(f"Error creating collection '{collection_name}': {e!s}")
            ...
            if "Collection not found" in str(e):
                try:
                    ...
                    logger.error(f"Error creating collection on second attempt: {e2!s}")
                    raise
            raise

    async def recreate_collection(self, collection_name: str | None = None,
                                vector_size: int = 1536) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Error recreating collection '{collection_name}': {e!s}")
            # Try again if collection doesn't exist
            if "Collection not found" in str(e):
                try:
                    await client.create_collection(
    ...
    async def delete_documents_by_site(
        self, site: str, collection_name: str | None = None
    ) -> int:
        """
    ...
    async def upload_documents(self, documents: list[dict[str, Any]],
        ...
        try:
            ...
            if points:
                ...
                for i in range(0, len(points), batch_size):
                    ...
                    try:
                        ...
                        logger.error(f"Error uploading batch: {e!s}")
            ...
            logger.exception(f"Error uploading documents to collection '{collection_name}': {e!s}")
            raise
    ...
    async def search(self, query: str, site: Union[str, list[str]],
        ...
        try:
            ...
            if collection_created:
                ...
                # Format the results
                results = self._format_results(query_response.points)
            ...
            logger.exception(f"Error in Qdrant search: {e!s}")
    ...
    async def get_sites(self, collection_name: str | None = None) -> list[str]:
        """
