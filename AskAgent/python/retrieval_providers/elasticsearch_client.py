...
class ElasticsearchClient(RetrievalClientBase):
    ...
    async def create_index_if_not_exists(self, index_name: str | None = None) -> bool:
        """
    ...
    async def _search_knn_filter(self, index_name: str, embedding: list[float],
                                 k: int, source: list[str], filter: dict[str, Any] | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            logger.exception("Error in Elasticsearch")
            logger.log_with_context(
                LogLevel.ERROR,
    ...
    async def search_all_sites(self, query: str, num_results: int = 50, query_params: dict[str, Any] | None = None, **kwargs) -> list[list[str]]:
        """
