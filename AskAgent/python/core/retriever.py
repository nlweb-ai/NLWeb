...
def init():
    ...
    for _endpoint_name, endpoint_config in CONFIG.retrieval_endpoints.items():
    ...
    for package in packages:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Failed to install required package {package} for {db_type}") from e
...
class VectorDBClientInterface(ABC):
    ...
    async def get_sites(self, **kwargs) -> list[str] | None:
        """
...
class RetrievalClientBase(VectorDBClientInterface):
    ...
    async def _get_cached_sites(self) -> list[str] | None:
        """
...
class VectorDBClient:
    ...
    def __init__(self, endpoint_name: str | None = None, query_params: dict[str, Any] | None = None):
        """
    ...
    async def get_client(self, endpoint_name: str) -> VectorDBClientInterface:
        ...
        async with _client_cache_lock:
            ...
            try:
                ...
                raise ValueError(f"Failed to load client for {db_type}: {e}") from e
    ...
    async def search(self, query: str, site: Union[str, list[str]],
                    num_results: int = 50, endpoint_name: str | None = None, **kwargs) -> list[list[str]]:
        """
        Search for documents matching the query and site.
        ...
        async with self._retrieval_lock:
            ...
            for endpoint_name, result in zip(endpoint_names, results, strict=False):
    ...
    async def search_by_url(self, url: str, endpoint_name: str | None = None, **kwargs) -> list[str] | None:
        """
        Retrieve a document by its exact URL.
    ...
    async def search_all_sites(self, query: str, num_results: int = 50,
                             endpoint_name: str | None = None, **kwargs) -> list[list[str]]:
        """
        Search across all sites.
    ...
    async def get_sites(self, endpoint_name: str | None = None, **kwargs) -> list[str]:
        """
        Get list of all sites available in the database.
        ...
        async with self._retrieval_lock:
            ...
            try:
                ...
                if sites is None:
                    ...
                    logger.info("Backend doesn't support get_sites, will query for all sites")
                    return []
...
def get_vector_db_client(endpoint_name: str | None = None,
                        query_params: dict[str, Any] | None = None) -> VectorDBClient:
    """
...
async def get_sites(endpoint_name: str | None = None,
                   query_params: dict[str, Any] | None = None) -> list[str]:
    """
