    ...
    if schema_json is None:
        ...
        if (should_skip_item(site, schema_json)):
            ...
            for k, v in schema_json.items():
                ...
                if k == "image" and isinstance(v, list) and len(v) > 0 and all(isinstance(img, str) for img in v):
                    retval[k] = v[0]
                    continue
                        ...
                        try:
                            ...
                            for item in schema_json:
                                try:
                                    ...
                                    print(f"Error processing item in binary mode for {url}: {e!s}")
                                    continue
                            ...
                            print(f"Error processing binary data from {url}: {e!s}")
            except UnicodeDecodeError as e:
                print(f"Failed to decode with {encoding}: {e!s}")
    except Exception as e:
        print(f"Binary processing failed: {e!s}")
...
def trim_schema_json_file(file_path, output_file):
    ...
    try:
        ...
        if encoding and encoding.startswith('utf-16'):
            print("Using binary processing for UTF-16 encoded file")
             ...
             open(output_file, 'w', encoding='utf-8') as out:
                for line in f:
                    ...
                    try:
                        ...
                        print(f"JSON decode error for {url}: {e!s}")
                        continue
                    except Exception as e:
                        print(f"Error processing {url}: {e!s}")
                        continue
    except UnicodeDecodeError as e:
        print(f"Encoding detection failed, trying binary mode: {e!s}")
