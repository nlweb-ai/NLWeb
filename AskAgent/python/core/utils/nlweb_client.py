...
async def ask_nlweb_server(
    ...
    try:
        ...
        logger.error(f"Error making request to {endpoint}: {e!s}", exc_info=True)
        raise
...
async def ask_nlweb_streaming(
    ...
    **kwargs
    ...
    async with aiohttp.ClientSession() as session, session.get(endpoint, params=params) as response:
        if response.status != 200:
            error_text = await response.text()
            raise Exception(f"Request failed with status {response.status}: {error_text}")
...
async def ask_endpoint(url: str, query: str, site: str | None = None, streaming: bool = False, **kwargs) -> list[dict[str, Any]]:
    """
