...
class Tool:
    ...
    schema_type: str
    prompt: str
    return_structure: dict[str, Any] | None = None
...
class ToolSelector:
        ...
        try:
            ...
            for completed_task in asyncio.as_completed(tasks):
                try:
                    ...
                    print(f"[TOOL-SELECTOR] Error processing completed task: {e!s}")
                    logger.error(f"Error processing completed task: {e!s}", exc_info=True)
                    pass
    ...
    async def do(self):
        ...
        try:
            ...
            if tool_results:
                ...
                for _i, result in enumerate(tool_results):
    ...
    async def _evaluate_tool(self, query: str, tool: Tool) -> dict:
        ...
        try:
            ...
            logger.error(f"Tool evaluation error for {tool.name}: {e!s}")
            return {"tool": tool, "score": 0, "result": {"score": 0, "justification": f"Error: {e!s}"}}
