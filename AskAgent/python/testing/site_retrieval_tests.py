...
class SiteRetrievalTestCase(TestCase):
    ...
    expected_min_sites: int | None = None
    expected_max_sites: int | None = None
    contains_sites: list[str] | None = None
    excludes_sites: list[str] | None = None
...
class SiteRetrievalTestRunner(BaseTestRunner):
    ...
    def validate_test_case(self, test_case: dict[str, Any]) -> tuple[bool, str | None]:
        ...
        if test_case.get('expected_sites') and not isinstance(test_case['expected_sites'], list):
            return False, "expected_sites must be a list"

        # Validate contains_sites format if present
        if test_case.get('contains_sites') and not isinstance(test_case['contains_sites'], list):
            return False, "contains_sites must be a list"

        # Validate excludes_sites format if present
        if test_case.get('excludes_sites') and not isinstance(test_case['excludes_sites'], list):
    ...
    async def run_single_test(self, test_case: dict[str, Any]) -> SiteRetrievalTestResult:
        ...
        try:
            ...
            if site_case.expected_sites is not None and set(sites) != set(site_case.expected_sites):
