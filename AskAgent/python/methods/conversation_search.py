    ...
    async def send_answers(self, force=False):
        ...
        if json_results:  # Only attempt to send if there are results
            ...
            try:
                ...
                logger.error(f"Client disconnected while sending answers: {e!s}")
                self.handler.connection_alive_event.clear()
            except Exception as e:
                logger.error(f"Error sending answers: {e!s}")
                self.handler.connection_alive_event.clear()
...
class ConversationSearchHandler:
    ...
    async def do(self):
        ...
        try:
            ...
            user_id = None
            if not search_all_users and hasattr(self.handler, 'query_params') and self.handler.query_params:
