...
class StatisticsHandler:
    ...
    async def match_templates(self, query: str, threshold: int = 70) -> list[dict]:
        ...
        if matched_templates:
            ...
            if len(matched_templates) > 3:
                ...
                logger.info("Limited to top 3 templates")
    ...
    async def process_template(self, match: dict, query: str) -> dict | None:
        ...
        for var_name in template_extract:
    ...
    def create_web_component(self, component_type: str, places: list[str], variables: list[str],
                           title: str = "", query_params: dict | None = None,
        ...
        if component_type == 'datacommons-line':
            ...
            if len(places) == 0 or any('counties' in p.lower() or 'us' in p.lower() for p in places):
                ...
                component_html += ' parentPlace="country/USA"'
                component_html += ' childPlaceType="County"'
            ...
            if len(places) == 0 or any('counties' in p.lower() or 'us' in p.lower() for p in places):
                ...
                # If we have a specific state/place, use it as parent
                component_html += f' parentPlace="{places[0]}"'
                component_html += ' childPlaceType="County"'  # Default to County
            if variables:
        ...
        try:
            ...
            for component in template_results:
                ...
                if component['html'] in seen_html:
                    logger.debug("Skipping duplicate component with identical HTML")
                    continue
            ...
            for _i, comp in enumerate(all_components):
                message = {
                    "message_type": "result",
            ...
            await self._send_error_message(f"An error occurred while processing your statistical query: {e!s}")
