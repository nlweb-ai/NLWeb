...
from core.utils.nlweb_client import ask_nlweb_streaming, sites_from_who_streaming
...
class MultiSiteQueryHandler:
    ...
    async def do(self):
        ...
        try:
            ...
            if sites_to_query:
                ...
                print("[MULTI-SITE] WARNING: No sites returned from who endpoint!")
            ...
            print(f"[MULTI-SITE] ERROR: {e!s}")
            import traceback
            print(f"[MULTI-SITE] Traceback: {traceback.format_exc()}")
            logger.error(f"Error in multi-site query: {e!s}", exc_info=True)
            await self._send_error_message(f"Error during multi-site query: {e!s}")
            return []
    ...
    async def _query_site_and_stream(self, domain: str, site_info: dict[str, Any], base_url: str):
        ...
        try:
            ...
            logger.error(f"Error querying site {domain}: {e!s}")
