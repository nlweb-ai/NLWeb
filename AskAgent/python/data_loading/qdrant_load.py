...
from core.retriever import upload_documents as upload_documents_wrapper
...
async def recreate_collection(client, collection_name: str, vector_size: int):
    ...
    print("WARNING: Collection recreation is not supported through the generic interface.")
    print("Please use database-specific tools to recreate collections.")
        ...
        for line in file:
            if line.strip():
                try:
                    ...
                    print(f"Skipping row due to error: {e!s}")
    return documents


async def upload_documents_to_database(documents: list[dict[str, Any]], database: str | None = None):
    """Upload documents to the configured write endpoint or specified database"""
    # Filter out documents without embeddings
    valid_documents = [doc for doc in documents if doc.get("embedding")]
...
async def upload_data_from_csv(csv_file_path: str, site: str, database: str | None = None):
