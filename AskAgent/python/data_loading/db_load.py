...
from core.retriever import delete_documents_by_site, get_vector_db_client, upload_documents
...
def process_line(line):
    ...
    if len(parts) >= 2:
        ...
        try:
            ...
            print(f"Error extracting URL from JSON: {e!s}")
            return None, None
    else:
        return None, None
...
async def delete_site_from_database(site: str, database: str | None = None):
    """
    ...
    try:
        async with aiohttp.ClientSession() as session, session.get(url) as response:
            ...
            if 'application/json' in content_type:
                ...
                ext = '.csv'
            elif 'application/rss+xml' in content_type or 'application/atom+xml' in content_type or 'text/xml' in content_type:
                ext = '.xml'
        ...
        print(f"Error fetching URL {url}: {e!s}")
        raise
...
async def detect_file_type(file_path: str) -> tuple[str, bool]:
    ...
    if await is_url(file_path):
        _temp_path, file_type = await save_url_content(file_path)
...
async def process_csv_file(file_path: str, site: str) -> list[dict[str, Any]]:
    ...
    try:
        with open(file_path, encoding='utf-8', errors='replace') as csv_file:
            ...
            try:
                ...
                if len(csv_reader.fieldnames or []) < 2:
                    print("Warning: CSV file has fewer than 2 columns. This may not contain enough data.")
            except Exception as e:
            ...
            for index, row in enumerate(csv_reader):
                try:
                    ...
                    error_count += 1
                    print(f"Error processing row {index}: {row_error!s}")
        ...
        print(f"Fatal error processing CSV file: {e!s}")
        traceback.print_exc()
...
async def process_rss_feed(file_path: str, site: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"Error processing RSS/Atom feed: {e!s}")
        traceback.print_exc()
        return []

async def loadJsonWithEmbeddingsToDB(file_path: str, site: str, batch_size: int = 100, delete_existing: bool = False, database: str | None = None):
    """
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            _file_type, has_embeddings = await detect_file_type(file_path)
            if has_embeddings:
        ...
        for i, line in enumerate(lines):
            try:
                ...
                if (len(batch_documents) >= batch_size or i == total_lines - 1) and batch_documents:
                ...
                print(f"Error processing line {i+1}: {e!s}")
...
async def loadJsonToDB(file_path: str, site: str, batch_size: int = 100, delete_existing: bool = False, force_recompute: bool = False, database: str | None = None):
    """
    ...
    try:
        ...
        if has_embeddings and not force_recompute:
            print("File already contains embeddings, switching to direct loading mode...")
                ...
                for i in range(0, len(all_documents), batch_size):
                    ...
                    if batch_docs and batch_texts:
                        try:
                            ...
                            print(f"Error processing batch: {e!s}")
                            traceback.print_exc()
...
async def loadUrlListToDB(file_path: str, site: str, batch_size: int = 100, delete_existing: bool = False, force_recompute: bool = False, database: str | None = None):
    """
    ...
    try:
        ...
        query_params = {"db": database} if database else None
        get_vector_db_client(query_params=query_params)
        ...
        for i, url in enumerate(urls):
            ...
            try:
                ...
                try:
                    ...
                    file_type, _has_embeddings = await detect_file_type(temp_url_path)
                    print(f"Detected file type: {file_type}")
                ...
                print(f"Error processing URL {url}: {e!s}")
                traceback.print_exc()
        ...
        print(f"Error reading URL list file: {e!s}")
        traceback.print_exc()
        return 0
    finally:
...
async def delete_site(site: str, database: str | None = None):
    """
...
async def process_normal_path(input_file_path: str, site: str, batch_size: int = 100, delete_site: bool = False, force_recompute: bool = False, database: str | None = None):
