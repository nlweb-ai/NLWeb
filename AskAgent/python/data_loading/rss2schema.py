...
def safe_get_text(element: ET.Element | None) -> str:
    """
...
def extract_duration(duration_str: str) -> str | None:
    """
...
def extract_guid(item: ET.Element) -> str | None:
    """
...
def extract_best_url(item: ET.Element, feed_url: str | None = None) -> str | None:
    """
...
def parse_rss_2_0(root: ET.Element, feed_url: str | None = None) -> list[dict[str, Any]]:
    """
    ...
    for item in channel.findall('item'):
        try:
            ...
            print(f"Error processing RSS item: {e!s}")
            traceback.print_exc()

    return result

def parse_atom(root: ET.Element, feed_url: str | None = None) -> list[dict[str, Any]]:
    """
    ...
    for entry in root.findall('atom:entry', ns):
        try:
            ...
            print(f"Error processing Atom entry: {e!s}")
            traceback.print_exc()
...
def feed_to_schema(feed_path: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"Error converting feed to schema: {e!s}")
        traceback.print_exc()
        return []
