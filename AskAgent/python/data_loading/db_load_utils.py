...
SKIP_TYPES = ["ItemList", "ListItem", "AboutPage", "WebPage", "WebSite", "Person", "BreadcrumbList"]
...
def should_include_item(js):
    ...
    if "@type" in js:
        item_type = js["@type"]
        if isinstance(item_type, list) and any(t in INCLUDE_TYPES for t in item_type):
            return True
...
def prepare_documents_from_json(url: str, json_data: str, site: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    try:
        ...
        print(f"Error preparing documents from JSON: {e!s}")
        return [], []
...
async def upload_batch_to_db(client, db_type, documents, batch_idx, total_batches, endpoint_name=None):
    ...
    try:
        ...
        print(f"Error uploading batch {batch_idx+1}: {e!s}")
        import traceback
        traceback.print_exc()
