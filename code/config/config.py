...
class NLWebConfig:
    ...
    # Default folder for JSON with embeddings
    json_with_embeddings_folder: str = "./data/json_with_embeddings"
    chatbot_instructions: Dict[str, str] = field(
        default_factory=dict)  # Dictionary of chatbot instructions
    ...
    headers: Dict[str, str] = field(
        default_factory=dict)  # HTTP headers to send
...
class AppConfig:
    ...
    def _get_base_output_directory(self) -> Optional[str]:
        ...
        if base_dir and not os.path.exists(base_dir):
            try:
                ...
                print(
                    f"Warning: Failed to create output directory {base_dir}: {e}")
                return None
        return base_dir
    ...
    def load_llm_config(self, path: str = "config_llm.yaml"):
        ...
        with open(full_path, "r") as f:
            ...
            for name, cfg in data.get("endpoints", {}).items():
                ...
                api_endpoint = self._get_config_value(
                    cfg.get("api_endpoint_env"))
                api_version = self._get_config_value(
                    cfg.get("api_version_env"))
    ...
    def load_embedding_config(self, path: str = "config_embedding.yaml"):
        ...
        try:
            ...
            print(
                f"Warning: {path} not found. Using default embedding configuration.")
            data = {
    ...
    def load_retrieval_config(self, path: str = "config_retrieval.yaml"):
        ...
        try:
            ...
            # If config file doesn't exist, use defaults
            print(
                f"Warning: {path} not found. Using default retrieval configuration.")
            data = {
                "preferred_endpoint": "default",
                "endpoints": {}
            }

        # Store the preferred retrieval endpoint
        self.preferred_retrieval_endpoint: str = data.get(
    ...
    def load_webserver_config(self, path: str = "config_webserver.yaml"):
        ...
        try:
            ...
            # If config file doesn't exist, use defaults
            print(
                f"Warning: {path} not found. Using default webserver configuration.")
            data = {
                "port": 8080,
        ...
        self.static_directory: str = self._get_config_value(
            data.get("static_directory"), "./static")
        ...
        if not os.path.isabs(self.static_directory):
            self.static_directory = os.path.abspath(
                os.path.join(config_dir, self.static_directory))
        ...
        logging_file = self._get_config_value(
            logging_data.get("file"), "./logs/webserver.log")
        ...
        static_config = StaticConfig(
            enable_cache=self._get_config_value(
                static_data.get("enable_cache"), True),
            cache_max_age=self._get_config_value(
                static_data.get("cache_max_age"), 3600),
            gzip_enabled=self._get_config_value(
                static_data.get("gzip_enabled"), True)
        )

        # Create the server config
        self.server = ServerConfig(
            host=self._get_config_value(server_data.get("host"), "localhost"),
            enable_cors=self._get_config_value(
                server_data.get("enable_cors"), True),
            max_connections=self._get_config_value(
                server_data.get("max_connections"), 100),
    ...
    def load_nlweb_config(self, path: str = "config_nlweb.yaml"):
        ...
        try:
            ...
            # If config file doesn't exist, use defaults
            print(
                f"Warning: {path} not found. Using default NLWeb configuration.")
            data = {
                "sites": "",
                "data_folders": {
                    "json_data": "./data/json",
        ...
        sites_list = [site.strip()
                      for site in sites_str.split(",") if site.strip()]
        ...
        tool_selection_enabled = self._get_config_value(
            data.get("tool_selection_enabled"), True)

        # Load memory enabled flag
        memory_enabled = self._get_config_value(
            data.get("memory_enabled"), False)

        # Load analyze query enabled flag
        analyze_query_enabled = self._get_config_value(
            data.get("analyze_query_enabled"), False)

        # Load decontextualize enabled flag
        decontextualize_enabled = self._get_config_value(
            data.get("decontextualize_enabled"), True)

        # Load required info enabled flag
        required_info_enabled = self._get_config_value(
        ...
        if base_output_dir:
            if not os.path.isabs(json_data_folder):
                json_data_folder = os.path.join(
                    base_output_dir, "data", "json")
            if not os.path.isabs(json_with_embeddings_folder):
                json_with_embeddings_folder = os.path.join(
                    base_output_dir, "data", "json_with_embeddings")
    ...
    def set_mode(self, mode: str):
        ...
        if mode.lower() not in ['development', 'production', 'testing']:
            raise ValueError(
                f"Invalid mode: {mode}. Must be 'development', 'production', or 'testing'")
