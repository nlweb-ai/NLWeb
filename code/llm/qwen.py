...
"""
Qwen wrapper for LLM functionality.
...
class QwenProvider(LLMProvider):
    """Implementation of LLMProvider for Qwen API."""
    ...
    def get_api_key(cls) -> str:
        """
        Retrieve the Qwen API key from environment or raise an error.
        """
        # Get the API key from the preferred provider config
        provider_config = CONFIG.llm_endpoints["qwen"]
        api_key = provider_config.api_key
        return api_key

    @classmethod
    def get_base_url_key(cls) -> str:
        """
        Retrieve the Qwen API key from environment or raise an error.
        """
        # Get the API key from the preferred provider config
    ...
    def get_client(cls) -> AsyncOpenAI:
        ...
        with cls._client_lock:  # Thread-safe client initialization
            if cls._client is None:
                ...
                base_url = cls.get_base_url_key()
                cls._client = AsyncOpenAI(base_url=base_url,api_key=api_key)
        return cls._client
    ...
    async def get_completion(
        ...
        **kwargs
        ...
        if model is None:
            provider_config = CONFIG.llm_providers["qwen"]
